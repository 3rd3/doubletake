...
func (c *AirPlayClient) setupMirrorSession(ctx context.Context, cfg StreamConfig) (*MirrorSession, error) {
	...
	// the receiver's playout buffer lead. Roku needs the conservative floor;
	// AppleTV2/3 need the smaller legacy floor so RECORD's ~1.2ms hardware
	// delay cannot collapse the buffer. Modern Apple receivers can run at the
	// configured target unchanged.
	...
	recordSession := func() error {
		...
		if value, ok := responseHeaders["audio-latency"]; ok {
			...
			if parseErr != nil {
				...
				prevSamples := audioLatencySamples
				audioLatencySamples, sessionLatency = adoptRecordAudioLatency(
					audioLatencySamples, sessionLatency, c.info.playoutLatencyFloor(), parsed)
				dbg("[SETUP] receiver Audio-Latency: %d samples; session lead %d -> %d samples (%v)",
					parsed, prevSamples, audioLatencySamples, sessionLatency)
			}
		}
		return nil
	}
